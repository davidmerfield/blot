...
# Create proxy only; start.sh toggles latency based on server logs (fast startup/restarts).
echo "[toxiproxy] Configuring redis proxy via API (latency toggled after server readiness logs) ..."
