xdescribe("Blot dashboard settings", function () {
  ...
  it("lets you update the title of a site", async function () {
    ...
    console.log(
      ...
      'a[href="/sites/' + this.blog.handle + '/title"]',
    );
  ...
  it("lets you sync the folder using local folder", async function () {
    ...
    expect(
      await page.$eval(
        'a[href^="/sites/' + this.blog.handle + '/client"] span.center',
    ...
    console.log(
      "writing a test file to the blog folder:",
      blogFolder + "/test.txt",
    );
    ...
    expect(
      await page.$eval("table.directory-list", (el) => el.innerText.trim()),
    ).toContain("test.txt");
