describe("replaceFolderLinks", function () {
    ...
    it("should replace src attributes with versioned CDN URLs", async function () {
        ...
        expect(result).toMatch(
            new RegExp(`<img src="${config.cdn.origin}/folder/v-[a-f0-9]{8}/[^"]+/images/test.jpg">`)
        );
    });
    ...
    it("should replace href attributes with versioned CDN URLs", async function () {
        ...
        expect(result).toMatch(
            new RegExp(`<a href="${config.cdn.origin}/folder/v-[a-f0-9]{8}/[^"]+/docs/test.pdf">Download</a>`)
        );
    });
    ...
    it("should handle multiple replacements in the same document", async function () {
        ...
        expect(result).toMatch(new RegExp(`${config.cdn.origin}/folder/v-[a-f0-9]{8}`));
        expect(result.match(new RegExp(`${config.cdn.origin}/folder/v-[a-f0-9]{8}`, 'g')).length).toEqual(2);
    });
    ...
    it("should handle multiple attributes in the same tag", async function () {
        ...
        const matches = result.match(new RegExp(`${config.cdn.origin}/folder/v-[a-f0-9]{8}`, 'g'));
