describe("entry.getByUrl", function () {
  ...
  it("normalizes fragment links to the canonical pathname", function (done) {
    const calls = [];
    ...
    withMocks(
      ...
      (_blogID, _entryID, callback) => callback({ id: "/entry.txt" }),
      getByUrl => {
        getByUrl("blog-id", "/target#section", entry => {
          expect(calls.length).toEqual(1);
          expect(calls[0]).toContain("/target");
          expect(calls[0]).not.toContain("#section");
          expect(entry.id).toEqual("/entry.txt");
          done();
        });
      }
  ...
  it("normalizes query links to the canonical pathname", function (done) {
    const calls = [];
    ...
    withMocks(
      ...
      (_blogID, _entryID, callback) => callback({ id: "/entry.txt" }),
      getByUrl => {
        getByUrl("blog-id", "/target?x=1", entry => {
          expect(calls.length).toEqual(1);
          expect(calls[0]).toContain("/target");
          expect(calls[0]).not.toContain("?x=1");
          expect(entry.id).toEqual("/entry.txt");
          done();
        });
      }
  ...
  it("keeps encoded unicode decoding behavior while removing query and fragment", function (done) {
    const calls = [];
    ...
    withMocks(
      ...
      (_blogID, _entryID, callback) => callback({ id: "/entry.txt" }),
      getByUrl => {
        getByUrl("blog-id", "/gr%C3%BC%C3%9Fe?x=1#section", entry => {
          expect(calls.length).toEqual(1);
          expect(calls[0]).toContain("/grüße");
          expect(calls[0]).not.toContain("?x=1");
          expect(calls[0]).not.toContain("#section");
