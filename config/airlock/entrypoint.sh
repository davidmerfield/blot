...
#   2. Start the tinyproxy forward proxy        (:8888).
#   3. Start headless Chromium (DevTools on 127.0.0.1:9221).
#   4. Start nginx in front of Chromium         (:9222, all interfaces).
#
...
BROWSER_PORT="${AIRLOCK_BROWSER_PORT:-9222}"
CHROMIUM_PORT="${AIRLOCK_CHROMIUM_PORT:-9221}"
PROXY_PORT="${AIRLOCK_PROXY_PORT:-8888}"
...
# nginx needs these temp dirs to exist and be writable by the airlock user.
...
PROXY_PID=$!
echo "airlock: tinyproxy started on :${PROXY_PORT} (pid ${PROXY_PID})"
...
CHROMIUM_PID=$!
echo "airlock: chromium started on 127.0.0.1:${CHROMIUM_PORT} (pid ${CHROMIUM_PID})"
...
# ---------------------------------------------------------------------------
su-exec airlock:airlock nginx -c /etc/airlock/nginx.conf -e stderr \
    -g "daemon off; pid /tmp/nginx.pid;" &
NGINX_PID=$!
echo "airlock: nginx front started on :${BROWSER_PORT} (pid ${NGINX_PID})"
